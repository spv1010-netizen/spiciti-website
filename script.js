...
function buildUpiUri(settings, checkout) {
  ...
  return "upi://pay?pa=" + upiId
    + "&pn=Spiciti"
    + "&am=" + amount.toFixed(2)
    + "&tn=Spiciti Order " + orderReference
    + "&cu=INR";
}
...
function getCheckoutInstructionText() {
  ...
  if (state.checkoutModal.platform === "android") {
    return "Tap Open UPI App to continue payment";
  }
...
function updateCheckoutActionButtons() {
  ...
  setCheckoutNodesHidden("[data-checkout-manual-upi]", !hasUpi || !isIOS);
  ...
  setCheckoutNodesHidden("[data-checkout-copy-upi]", !hasUpi || !isIOS);
  setCheckoutNodesDisabled("[data-checkout-copy-upi]", !hasUpi || !isIOS);
