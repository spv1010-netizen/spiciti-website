...
function getCurrentPageId() {
  return String(document.body && document.body.getAttribute("data-page") || "").toLowerCase();
}
...
function stopOfferTickerAnimation() {
  if (state.offerTicker.frameId) {
    window.cancelAnimationFrame(state.offerTicker.frameId);
    state.offerTicker.frameId = 0;
  }

  if (state.offerTicker.track) {
    state.offerTicker.track.style.transform = "";
    state.offerTicker.track.classList.remove("is-marquee-active");
  }

  state.offerTicker.offset = 0;
  state.offerTicker.lastTimestamp = 0;
  state.offerTicker.track = null;
}

function startOfferTickerAnimation(track) {
  if (!track) {
    stopOfferTickerAnimation();
    return;
  }

  stopOfferTickerAnimation();

  if (!track.dataset.loopDuplicated) {
    track.innerHTML += track.innerHTML;
    track.dataset.loopDuplicated = "true";
  }

  state.offerTicker.track = track;
  track.classList.add("is-marquee-active");

  function animateOfferTicker(timestamp) {
    var activeTrack = state.offerTicker.track;
    var resetWidth = activeTrack ? activeTrack.scrollWidth / 2 : 0;
    var elapsed = 0;
    var pixelsPerSecond = window.innerWidth <= 760 ? 30 : 42;

    if (!activeTrack || !resetWidth) {
      stopOfferTickerAnimation();
      return;
    }

    if (document.hidden) {
      state.offerTicker.lastTimestamp = timestamp;
      state.offerTicker.frameId = window.requestAnimationFrame(animateOfferTicker);
      return;
    }

    if (!state.offerTicker.lastTimestamp) {
      state.offerTicker.lastTimestamp = timestamp;
    }

    elapsed = Math.min(48, timestamp - state.offerTicker.lastTimestamp);
    state.offerTicker.lastTimestamp = timestamp;
    state.offerTicker.offset -= (pixelsPerSecond * elapsed) / 1000;

    if (Math.abs(state.offerTicker.offset) >= resetWidth) {
      state.offerTicker.offset += resetWidth;
    }

    activeTrack.style.transform = "translate3d(" + state.offerTicker.offset.toFixed(2) + "px, 0, 0)";
    state.offerTicker.frameId = window.requestAnimationFrame(animateOfferTicker);
  }
...
function getStoreSettings() {
  const publishedSettings = getSiteContentValue("settings") || {};
  const localSettings = safeRead(STORAGE_KEYS.settings, {});
  ...
  if (getCurrentPageId() !== "admin" && publishedSettings && typeof publishedSettings === "object") {
    if (typeof publishedSettings.showOfferTicker === "boolean") {
      mergedSettings.showOfferTicker = publishedSettings.showOfferTicker;
    }

    if (publishedSettings.offerTickerText !== undefined && publishedSettings.offerTickerText !== null) {
      mergedSettings.offerTickerText = String(publishedSettings.offerTickerText).trim();
    }
  }
...
function renderOfferTicker() {
  ...
  startOfferTickerAnimation(track);
}
...
function serializeProductForPersistence(product, index) {
  ...
  const persisted = {
    ...
    art: normalized.art,
    images: normalizeImageList(normalized.images),
    quantityOptions: normalized.quantityOptions.map(function (option) {
      return {
        id: option.id,
        label: option.label,
  ...
  const badgeText = normalizeBadge(normalized.badge);

  if (badgeText) {
    persisted.badge = badgeText;
  }

  return persisted;
}

function serializeCatalogForPersistence(products) {
  return (Array.isArray(products) ? products : [])
    .map(function (product, index) {
      return serializeProductForPersistence(product, index);
    })
    .filter(Boolean);
}
...
function saveCatalog(products) {
  const persistedCatalog = serializeCatalogForPersistence(products);

  saveValue(STORAGE_KEYS.catalog, persistedCatalog);
  if (siteContentCache && typeof siteContentCache === "object") {
    siteContentCache.catalog = persistedCatalog;
  }
...
function createBlogCard(post) {
  ...
  const hasImage = !hasVideo && Boolean(String(post.image || "").trim());
...
function startProductEdit(productId) {
  ...
  form.elements.badge.value = normalizeBadge(product.badge);
...
function createProductVisualMarkup(product, interactive) {
  ...
  const thumbnailMarkup = !interactive && images.length > 1
    ...
    : "";
  const visualMarkup = '<div class="' + visual.className + '"' + visual.styleAttribute + '>' + badgeMarkup + stockBadgeMarkup + '<div class="product-visual-copy"><p>Spiciti</p><strong>' + escapeHtml(product.name) + '</strong></div></div>' + thumbnailMarkup;
...
function buildUpiUri(settings, checkout) {
  var upiId = String(settings && settings.upiId || "").trim();
  var orderReference = String(checkout && checkout.reference || "").trim();
  var amount = Number(checkout && checkout.total || 0);

  if (!upiId || !orderReference || !(amount > 0)) {
    return "";
  }

  return "upi://pay?pa=" + encodeURIComponent(upiId)
    + "&pn=" + encodeURIComponent("Spiciti")
    + "&am=" + encodeURIComponent(amount.toFixed(2))
    + "&tn=" + encodeURIComponent("Spiciti Order " + orderReference)
    + "&cu=INR";
}
...
function buildCheckoutMessage(checkout, paymentStatus, settings) {
  ...
  const subtotal = Number(checkout && checkout.subtotal || 0);
  const shipping = Number(checkout && checkout.shipping || 0);
  const discount = Number(checkout && checkout.discount || 0);
  const total = Number(checkout && checkout.total || 0);
  const normalizedPaymentStatus = String(paymentStatus || checkout.paymentStatus || "Paid").trim() || "Paid";
  ...
  const lines = [
    ...
    "",
    "Cost Breakdown:",
    "Base Price: " + formatCurrency(subtotal),
    "Delivery Charge: " + formatCurrency(shipping),
    "",
    "Items:"
  ];

  if (discount > 0) {
    lines.splice(lines.indexOf("Items:"), 0, "Discount: - " + formatCurrency(discount), "Total Amount: " + formatCurrency(total), "");
  } else {
    lines.splice(lines.indexOf("Items:"), 0, "Total Amount: " + formatCurrency(total), "");
  }
...
function clearCheckoutActionTimers() {
  clearCheckoutCountdownTimers();

  if (state.checkoutModal.whatsappFallbackTimerId) {
    clearTimeout(state.checkoutModal.whatsappFallbackTimerId);
    state.checkoutModal.whatsappFallbackTimerId = 0;
  }
}

function setCheckoutNodesText(selector, text) {
  Array.prototype.forEach.call(document.querySelectorAll(selector), function (node) {
    node.textContent = text;
  });
}

function setCheckoutNodesHidden(selector, hidden) {
  Array.prototype.forEach.call(document.querySelectorAll(selector), function (node) {
    node.hidden = !!hidden;
  });
}

function setCheckoutNodesDisabled(selector, disabled) {
  Array.prototype.forEach.call(document.querySelectorAll(selector), function (node) {
...
function setCheckoutQrImage(upiUri, orderReference) {
  Array.prototype.forEach.call(document.querySelectorAll("[data-checkout-qr]"), function (node) {
    if (!upiUri) {
      node.removeAttribute("src");
      node.setAttribute("alt", "UPI payment QR code unavailable");
      return;
    }

    node.setAttribute("src", getUpiQrImageUrl(upiUri));
    node.setAttribute("alt", "UPI payment QR code for order " + orderReference);
  });
}

function getCheckoutCountdownSeconds(deviceType) {
  return deviceType === "desktop" ? 30 : 12;
}

function getCheckoutInstructionText() {
  if (!state.checkoutModal.upiUri) {
    return "UPI payment is not available right now.";
  }

  if (state.checkoutModal.platform === "ios") {
    return "Open your UPI app and pay using this UPI ID";
  }

  if (state.checkoutModal.platform === "android") {
    return "Tap Open UPI App, or pay manually using the UPI ID shown below";
  }

  return "Scan via UPI, or use the UPI ID shown below";
}

function getCheckoutPaymentStatusText() {
  return state.checkoutModal.readyToConfirm
    ? "Payment Status: Ready to confirm order"
    : "Payment Status: Waiting for payment";
}

function getCheckoutCountdownText() {
  if (!state.checkoutModal.upiUri) {
    return "UPI payment is not available right now.";
  }

  if (state.checkoutModal.readyToConfirm) {
    return "You can confirm order now";
  }

  return "You can confirm order in " + state.checkoutModal.countdownSecondsLeft + " seconds";
}

function setPaymentStatusPill(statusText) {
  Array.prototype.forEach.call(document.querySelectorAll("[data-checkout-status]"), function (pill) {
    pill.textContent = statusText;
    pill.classList.toggle("is-paid", /ready|confirmed|completed/i.test(statusText));
  });
}

function updateCheckoutActionButtons() {
  var platform = state.checkoutModal.platform || "desktop";
  var isDesktop = platform === "desktop";
  var isAndroid = platform === "android";
  var hasUpi = !!state.checkoutModal.upiUri;
  var hasWhatsApp = !!state.checkoutModal.whatsappUrl && state.checkoutModal.whatsappUrl !== "#";
  var hasEmail = !!state.checkoutModal.emailUrl;
  var enableSendActions = state.checkoutModal.readyToConfirm && (hasWhatsApp || hasEmail);

  setCheckoutNodesHidden("[data-checkout-qr-wrapper]", !hasUpi || !isDesktop);
  setCheckoutNodesHidden("[data-checkout-manual-upi]", !hasUpi);

  setCheckoutNodesHidden("[data-checkout-open-upi]", !hasUpi || !isAndroid);
  setCheckoutNodesDisabled("[data-checkout-open-upi]", !hasUpi || !isAndroid);

  setCheckoutNodesHidden("[data-checkout-copy-upi]", !hasUpi);
  setCheckoutNodesDisabled("[data-checkout-copy-upi]", !hasUpi);

  setCheckoutNodesHidden("[data-checkout-open-whatsapp]", isDesktop || !hasWhatsApp);
  setCheckoutNodesDisabled("[data-checkout-open-whatsapp]", !enableSendActions || isDesktop || !hasWhatsApp);

  setCheckoutNodesHidden("[data-checkout-open-email]", !hasEmail);
  setCheckoutNodesDisabled("[data-checkout-open-email]", !state.checkoutModal.readyToConfirm || !hasEmail);
}

function renderCheckoutExperience() {
  var checkout = state.checkoutModal.checkout;
  ...
  setCheckoutNodesText("[data-checkout-order-id]", "Order ID: " + checkout.reference);
  setCheckoutNodesText("[data-checkout-amount]", formatCurrency(checkout.total));
  setCheckoutNodesText("[data-checkout-upi-id]", settings.upiId || "Not configured");
  setCheckoutNodesText("[data-checkout-upi-id-copy]", settings.upiId || "Not configured");
  setCheckoutNodesText("[data-checkout-upi-name]", settings.upiName || "Spiciti");
  setCheckoutNodesText("[data-checkout-instruction]", getCheckoutInstructionText());
  setCheckoutNodesText("[data-checkout-countdown]", getCheckoutCountdownText());
  setCheckoutNodesText("[data-checkout-manual-note]", state.checkoutModal.platform === "desktop"
    ? "If you are paying from another device, use this UPI ID in any UPI app."
    : "If the quick pay button does not work, use this UPI ID in any UPI app.");
  Array.prototype.forEach.call(document.querySelectorAll("[data-checkout-notice]"), function (node) {
    node.innerHTML = "<strong>Orders are processed after payment confirmation.</strong>";
  });
  setPaymentStatusPill(getCheckoutPaymentStatusText());
  setCheckoutQrImage(state.checkoutModal.upiUri, checkout.reference);
  updateCheckoutActionButtons();
}

function startCheckoutConfirmationCountdown(seconds) {
  clearCheckoutActionTimers();

  state.checkoutModal.readyToConfirm = false;
  state.checkoutModal.countdownSecondsLeft = Math.max(0, Number(seconds) || 0);
  renderCheckoutExperience();
  ...
  if (!state.checkoutModal.countdownSecondsLeft) {
    state.checkoutModal.readyToConfirm = true;
    renderCheckoutExperience();
    return;
  }
...
function buildCheckoutEmailUrl(checkout, paymentStatus, settings) {
...
function setCheckoutFlowState(checkout, mode) {
  var settings = getStoreSettings();
  var deviceType = getCheckoutDeviceType();
  var platform = getCheckoutPlatform();

  state.checkoutModal.mode = mode || "";
  state.checkoutModal.checkout = checkout || null;
  state.checkoutModal.deviceType = deviceType;
  state.checkoutModal.platform = platform;
  state.checkoutModal.upiUri = checkout ? buildUpiUri(settings, checkout) : "";
  state.checkoutModal.whatsappUrl = checkout
    ? buildWhatsAppUrl(settings.whatsappPhone, buildCheckoutMessage(checkout, "Paid", settings))
    : "";
  state.checkoutModal.emailUrl = checkout
    ? buildCheckoutEmailUrl(checkout, "Paid", settings)
    : "";
  state.checkoutModal.readyToConfirm = false;
  state.checkoutModal.countdownSecondsLeft = checkout ? getCheckoutCountdownSeconds(deviceType) : 0;
}

function openCheckoutSurface(checkout, mode) {
  if (!checkout) {
    return false;
  }

  setCheckoutFlowState(checkout, mode);
  startCheckoutConfirmationCountdown(state.checkoutModal.countdownSecondsLeft);
  return true;
}

function ensureCheckoutActionReady() {
  if (!state.checkoutModal.checkout) {
...
function closeCheckoutPaymentModal() {
  ...
  state.checkoutModal.mode = "";
  state.checkoutModal.checkout = null;
  state.checkoutModal.upiUri = "";
  state.checkoutModal.whatsappUrl = "";
  state.checkoutModal.emailUrl = "";
  state.checkoutModal.platform = "desktop";
...
function openCheckoutPaymentModal(checkout) {
  ...
  openCheckoutSurface(checkout, "modal");
...
function openUPI() {
  var upiURL = state.checkoutModal.upiUri;
  ...
  if (state.checkoutModal.platform !== "android") {
    if (state.checkoutModal.platform === "ios") {
      showToast("Use your UPI app with the UPI ID shown on screen.");
    } else {
      showToast("Scan the QR code to pay from your phone.");
    }
    return;
  }
...
function copyCheckoutUpiId() {
  ...
  if (!upiId) {
    showToast("UPI ID is not available right now.");
    return;
  }
  ...
  if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
    navigator.clipboard.writeText(upiId).then(function () {
      ...
      showToast("Unable to copy UPI ID.");
    });
    return;
  }
...
function handlePaymentVisibilityChange() {
  if (!state.checkoutModal.isOpen) {
    return;
  }
}

function openWhatsApp() {
  var whatsappURL = state.checkoutModal.whatsappUrl;
  var emailURL = state.checkoutModal.emailUrl;
  ...
  if (!whatsappURL || whatsappURL === "#") {
    if (emailURL) {
      ...
      showToast("WhatsApp is not available right now.");
    }
    return;
  }

  if (deviceType === "desktop") {
    showToast("Use the email option on desktop.");
    return;
  }

  updatePendingCheckoutStatus("Ready to confirm order");
  ...
  if (deviceType === "tablet") {
    ...
    state.checkoutModal.whatsappFallbackTimerId = window.setTimeout(function () {
      cleanup();
      if (!appSwitchDetected && document.hasFocus() && !document.hidden && emailURL) {
        window.location.href = emailURL;
      }
    }, 1200);
  }
...
function openEmail() {
  ...
  if (!emailURL) {
    showToast("Email confirmation is not available right now.");
    return;
  }
...
function renderCheckoutPage() {
  var summaryTarget = document.querySelector("[data-checkout-summary]");
  var checkout = ensurePendingCheckout();
  ...
  if (!checkout) {
    setCheckoutFlowState(null, "page");
    setCheckoutNodesText("[data-checkout-order-id]", "Order ID: -");
    setCheckoutNodesText("[data-checkout-amount]", "-");
    setCheckoutNodesText("[data-checkout-upi-id]", "Not configured");
    setCheckoutNodesText("[data-checkout-upi-id-copy]", "Not configured");
    setCheckoutNodesText("[data-checkout-upi-name]", "Spiciti");
    setCheckoutNodesText("[data-checkout-instruction]", "Scan via UPI");
    setCheckoutNodesText("[data-checkout-countdown]", "You can confirm order in 0 seconds");
    setCheckoutNodesText("[data-checkout-manual-note]", "Open your UPI app and pay using this UPI ID");
    setPaymentStatusPill("Payment Status: Waiting for payment");
    setCheckoutQrImage("", "");
    updateCheckoutActionButtons();
  ...
  openCheckoutSurface(checkout, "page");
}
...
function buildSiteBackupPayload() {
  return {
    catalog: serializeCatalogForPersistence(getCatalog()),
    reviews: getReviews(),
...
function prepareSiteContentForGitHub(rawContent) {
  ...
  content.catalog = (Array.isArray(content.catalog) ? content.catalog : [])
    .map(function (product, index) {
      const persistedProduct = serializeProductForPersistence(product, index);
      ...
      return {
        ...persistedProduct,
        images: normalizeImageList(persistedProduct.images).map(function (image, imageIndex) {
          return resolveImagePath(image, "products", persistedProduct.id || persistedProduct.name, String(imageIndex + 1));
        })
      };
    })
    .filter(Boolean);
...
function handleProductForm() {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    const normalizedProduct = {
      ...
      badge: normalizeBadge(formData.get("badge")),
...
function initialiseCheckoutActionButtons() {
  Array.prototype.forEach.call(document.querySelectorAll("[data-checkout-open-upi]"), function (button) {
  ...
  Array.prototype.forEach.call(document.querySelectorAll("[data-checkout-open-whatsapp]"), function (button) {
    button.addEventListener("click", function (event) {
      event.preventDefault();
      event.stopPropagation();
      openWhatsApp();
  ...
  Array.prototype.forEach.call(document.querySelectorAll("[data-checkout-open-email]"), function (button) {
    button.addEventListener("click", function (event) {
      event.preventDefault();
      event.stopPropagation();
      openEmail();
  ...
  Array.prototype.forEach.call(document.querySelectorAll("[data-checkout-copy-upi]"), function (button) {
    button.addEventListener("click", function (event) {
      event.preventDefault();
      event.stopPropagation();
      copyCheckoutUpiId();
...
document.addEventListener("DOMContentLoaded", function () {
  loadSiteContentFile().finally(function () {
    ...
    handleCustomerForm();
    handleContactForm();
    initialiseCheckoutActionButtons();
    renderCustomerForm();
    renderCartPage();
